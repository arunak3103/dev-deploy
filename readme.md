...
Developer → GitHub → CI/CD → Amazon Elastic Container Registry (ECR) → Amazon EKS → End User
...
* CI/CD uses secure authentication (OIDC) without storing credentials
* High availability using Multi-AZ database deployment
...
* Application and database run in private subnets
* Database is deployed in Multi-AZ mode for high availability
* Secrets are managed securely and not hardcoded
* Direct SSH access is disabled; access is managed using AWS Systems Manager (SSM)
...
* Image is stored in Amazon Elastic Container Registry (ECR)
...
* CI/CD uses OIDC to securely access AWS without storing long-term credentials
